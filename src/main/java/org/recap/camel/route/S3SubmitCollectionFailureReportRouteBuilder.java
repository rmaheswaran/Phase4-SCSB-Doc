...
public class S3SubmitCollectionFailureReportRouteBuilder {
    ...
    private static final Logger logger = LoggerFactory.getLogger(S3SubmitCollectionFailureReportRouteBuilder.class);
     ...
     */
    @Autowired
    public S3SubmitCollectionFailureReportRouteBuilder(CamelContext context, @Value("${add.s3.routes.on.startup}") boolean addS3RoutesOnStartup, @Value("${s3.submit.collection.report.dir}") String submitCollectionS3ReportPath) {
        try {
